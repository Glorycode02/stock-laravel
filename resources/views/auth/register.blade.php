...
<body>
    <div class="flex flex-col items-center justify-center py-28 gap-5">
        ...
        <form action="{{route("store")}}" method="post" class="flex flex-col items-center border border-slate-900 shadow-md rounded-md p-10 w-1/3 gap-4">
            @csrf
            @Method("POST")
            ...
            <div class="self-start flex gap-2 w-full">
                ...
                <input type="password" name="Cpass" value="{{old('Cpass')}}" class="outline-none border border-slate-500 rounded-md w-[96%]" required>
            </div>
            <input type="submit" value="Submit" class="p-2 w-full bg-blue-400 text-white rounded-md cursor-pointer hover:bg-blue-500">
        </form>
    </div>
</body>
